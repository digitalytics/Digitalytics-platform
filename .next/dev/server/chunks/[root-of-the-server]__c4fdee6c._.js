...
"[project]/app/api/campaigns/[id]/start/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function POST(_request, { params }) {
    ...
    const agent = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].agent.findUnique({
        where: {
            id: campaign.agentId
        }
    });
    ...
    const calledContactIds = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].outboundCall.findMany({
        where: {
            campaignId: id,
            status: {
                in: [
    ...
    }).then((calls)=>calls.map((c)=>c.contactId).filter(Boolean));
    const remainingContacts = campaign.contactList.members.map((m)=>m.contact).filter((c)=>!calledContactIds.includes(c.id));
    if (remainingContacts.length === 0) {
        ...
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            message: 'No remaining contacts — campaign marked complete'
        });
    }
    ...
    const results = await Promise.all(remainingContacts.map((contact)=>fireOneCall({
            campaignId: id,
            contact,
            ...
            dynamicVariables: campaign.dynamicVariables
        })));
    ...
    return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
        started: true,
        contactsToCall: remainingContacts.length,
        succeeded,
        failed
    });
}
async function fireOneCall({ campaignId, contact, agentId, agentRetellId, fromNumber, userId, dynamicVariables }) {
    ...
    try {
        const res = await fetch(`${process.env.RETELL_API_BASE_URL || 'https://api.retellai.com/v2'}/create-phone-call`, {
            method: 'POST',
            headers: {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__c4fdee6c._.js.map