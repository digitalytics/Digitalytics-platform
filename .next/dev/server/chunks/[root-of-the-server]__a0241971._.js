...
"[project]/lib/invoice.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function generateInvoiceNumber(year, monthIndex) {
    const mon = MONTH_ABBR[monthIndex];
    const prefix = `INV-${year}-${mon}-`;
    const count = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].invoice.count({
        where: {
            invoiceNumber: {
                startsWith: prefix
            }
...
async function generateInvoice(userId, periodStart, periodEnd, options = {}) {
    // 1. Fetch user agents
    ...
    const lineItems = [];
    const setupFeePaidUpdates = []; // userAgent IDs to mark setupFeePaid
    for (const ua of userAgents){
        const agentName = ua.agent.name;
        const agentId = ua.agentId;
        // a. Setup fee (only if not yet paid)
        if (ua.setupFee && ua.setupFee.greaterThan(0) && !ua.setupFeePaid) {
            lineItems.push({
                type: 'SETUP_FEE',
                agentId,
                agentName,
                description: `Agent Setup: ${agentName}`,
                quantity: new __TURBOPACK__imported__module__$5b$externals$5d2f40$prisma$2f$client__$5b$external$5d$__$2840$prisma$2f$client$2c$__cjs$2c$__$5b$project$5d2f$node_modules$2f40$prisma$2f$client$29$__["Prisma"].Decimal(1),
                unitPrice: ua.setupFee,
                total: ua.setupFee.toDecimalPlaces(2)
            });
            setupFeePaidUpdates.push(ua.id);
        }
        // b. Monthly fee
        if (ua.monthlyFee && ua.monthlyFee.greaterThan(0)) {
            lineItems.push({
                type: 'MONTHLY_FEE',
                agentId,
                agentName,
                description: `Monthly Service: ${agentName}`,
                quantity: new __TURBOPACK__imported__module__$5b$externals$5d2f40$prisma$2f$client__$5b$external$5d$__$2840$prisma$2f$client$2c$__cjs$2c$__$5b$project$5d2f$node_modules$2f40$prisma$2f$client$29$__["Prisma"].Decimal(1),
                unitPrice: ua.monthlyFee,
                total: ua.monthlyFee.toDecimalPlaces(2)
            });
        }
        // c. Usage fee
        if (ua.customPrice && ua.customPrice.greaterThan(0)) {
            const effectiveStart = ua.assignedAt > periodStart ? ua.assignedAt : periodStart;
            const result = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].call.aggregate({
                _sum: {
                    durationMs: true
                },
                where: {
            ...
            if (totalMs > 0) {
                const totalMinutes = new __TURBOPACK__imported__module__$5b$externals$5d2f40$prisma$2f$client__$5b$external$5d$__$2840$prisma$2f$client$2c$__cjs$2c$__$5b$project$5d2f$node_modules$2f40$prisma$2f$client$29$__["Prisma"].Decimal(totalMs).div(60000).toDecimalPlaces(2);
                if (totalMinutes.greaterThan(0)) {
                    const usageTotal = totalMinutes.mul(ua.customPrice).toDecimalPlaces(2);
                    lineItems.push({
                        type: 'USAGE_FEE',
                        agentId,
                        agentName,
                        description: `Usage: ${agentName} (${totalMinutes.toString()} min)`,
                        quantity: totalMinutes,
                        unitPrice: ua.customPrice,
                        total: usageTotal
                    });
                }
            }
    ...
    const subtotal = lineItems.reduce((sum, item)=>sum.plus(item.total), new __TURBOPACK__imported__module__$5b$externals$5d2f40$prisma$2f$client__$5b$external$5d$__$2840$prisma$2f$client$2c$__cjs$2c$__$5b$project$5d2f$node_modules$2f40$prisma$2f$client$29$__["Prisma"].Decimal(0)).toDecimalPlaces(2);
    const invoiceNumber = await generateInvoiceNumber(periodStart.getFullYear(), periodStart.getMonth());
    const invoice = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].$transaction(async (tx)=>{
        ...
        // Mark setup fees as paid
        if (setupFeePaidUpdates.length > 0) {
            await tx.userAgent.updateMany({
                where: {
                    id: {
                        in: setupFeePaidUpdates
                    }
                },
                data: {
                    setupFeePaid: true
...
var __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/lib/prisma.ts [app-route] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$invoice$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/lib/invoice.ts [app-route] (ecmascript)");
var __turbopack_async_dependencies__ = __turbopack_handle_async_dependencies__([
...
async function POST() {
    ...
    const activeUsers = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].user.findMany({
        where: {
            role: 'USER',
            status: 'ACTIVE'
        },
    ...
    // Find users who already have an invoice for this period
    const existing = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].invoice.findMany({
        where: {
            periodStart: {
                gte: periodStart
            },
    ...
    const alreadyInvoiced = new Set(existing.map((e)=>e.userId));
    let generated = 0;
    let skipped = 0;
    const errors = [];
    for (const user of activeUsers){
        if (alreadyInvoiced.has(user.id)) {
            skipped++;
            continue;
        }
        try {
            const invoice = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$invoice$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["generateInvoice"])(user.id, periodStart, periodEnd, {
            ...
            await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].notification.create({
                data: {
                    ...
                    title: 'New Invoice',
                    message: `Invoice ${invoice.invoiceNumber} has been generated for $${Number(invoice.total).toFixed(2)}.`,
                    link: '/billing'
                }
            });
            generated++;
...
//# sourceMappingURL=%5Broot-of-the-server%5D__a0241971._.js.map