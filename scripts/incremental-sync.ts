...
async function main() {
  ...
  try {
    logger.info('========================================');
    logger.info('INCREMENTAL SYNC - Retell AI Call Logs');
    logger.info('========================================');
    ...
    logger.info('Fetching only new calls since last sync');

    const result = await syncService.performIncrementalSync(targetAgents);
    ...
    if (result.totalCalls === 0) {
      logger.info('No new calls to sync. Database is up to date!');
    }
